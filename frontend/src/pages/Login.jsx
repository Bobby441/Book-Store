  ...
  const onSubmitHandler = async(e) => {
    ...
    try {
      if(currState === 'Sign Up') {
        ...
        console.log(response.data)
        if(response.data.success) {
          //console.log('Hello from success res')
          setToken(response.data.token)
          localStorage.setItem('token',response.data.token) 
        } else {
          toast.error(response.data.message)
            ...
            <div className='w-full flex flex-col gap-y-3 medium-14'>
              ...
              {currState === 'Login'? (
                ...
                <div className='underline'>
                  Already have an account? <span onClick={() => setCurrState('Login')} className='cursor-pointer hover:text-secondaryOne'span>Login</span>
                </div>
              )}
            </div>
          </form>
