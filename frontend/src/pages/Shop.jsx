...
const Shop = () => {
  ...
  const itemsPerPage = 10; // no.of books per page
  ...
  const applySorting = (booksList) => {
    switch (sortType) {
      ...
      default:
        return booksList; // default for relevance
    }
  };
      ...
      <div className="pt-28">
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {getPaginateBooks().length > 0 ? (
              getPaginateBooks().map((book) => (
                <Item book={book} key={book._id} />
              ))
            ) : (
              <p className="">No Books found for selected filters</p>
            )}
          </div>
        </div>
